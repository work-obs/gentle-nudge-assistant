...
import {
  ...
  createMinimalAnalyticsEngine,
  createHighVolumeAnalyticsEngine,
...
async function performanceMonitoringExample() {
  ...
  const engine = createHighVolumeAnalyticsEngine();
  
  console.log('Initial Cache Stats:', engine.getCacheStats());
  
  // Simulate some analysis work
  const mockIssues: JiraIssue[] = Array.from({ length: 50 }, (_, i) => ({
    id: `${i + 1}`,
    key: `TEST-${i + 1}`,
    fields: {
  ...
  try {
    ...
    const batchRequest = {
      ...
      analysisType: 'staleness_only' as const,
...
async function engineComparisonExample() {
  ...
  const engines = {
    minimal: createMinimalAnalyticsEngine(),
    standard: createAnalyticsEngine(),
    highVolume: createHighVolumeAnalyticsEngine()
  };
...
/**
 * Individual example runner functions for testing specific functionality
 */
export {
  basicAnalyticsExample,
  customConfigurationExample,
  batchAnalysisExample,
  projectAnalyticsExample,
  performanceMonitoringExample,
  engineComparisonExample,
  runAllExamples
};
