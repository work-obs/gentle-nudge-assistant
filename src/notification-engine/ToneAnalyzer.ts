...
export class ToneAnalyzer {
  ...
  async analyzeNotificationTone(
    ...
    try {
      ...
      const encouragementLevel = this.calculateEncouragementLevel(
        metrics,
        content
      );

      const score = this.calculateOverallToneScore(
        metrics,
        sentiment,
        encouragementLevel
      );
      ...
      const replacementSuggestions = this.generateReplacementSuggestions(
        content,
        flaggedWords
      );
  ...
  async validateContentTone(
    ...
    try {
      const analysisResult = await this.analyzeNotificationTone(
        content,
        requiredTone
      );
      ...
      if (requiredTone && analysis.tone !== requiredTone) {
        return {
          ...
          data: false,
          warnings: [
            `Content tone '${analysis.tone}' does not match required tone '${requiredTone}'`,
          ],
        };
      }
  ...
  async improveContentTone(
    ...
    try {
      ...
      if (analysis.encouragementLevel < 0.6) {
        improvedContent = await this.addEncouragingElements(
          improvedContent,
          targetStyle
        );
      }
      ...
      if (analysis.tone !== targetTone) {
        improvedContent = await this.adjustToTargetTone(
          improvedContent,
          analysis.tone,
          targetTone
        );
  ...
  async generateToneVariations(
    ...
    try {
      ...
      for (const tone of tones) {
        const variationResult = await this.improveContentTone(
          baseContent,
          tone
        );
  ...
  private initializeToneProfiles(): void {
    ...
    this.toneProfiles.set('casual', {
      ...
      negativeWords: ['sucks', 'lame', 'boring', 'meh', 'blah', 'ugh'],
      encouragingPhrases: [
        'no worries',
        'you got it',
        'easy does it',
      ...
      discouragingPhrases: ['you should', 'you need to', 'you have to'],
    ...
    this.toneRules = [
      {
        ...
        severity: 'error',
        suggestion:
          'Replace negative words with positive or neutral alternatives',
        autoFix: content => {
          let fixed = content;
          const negativeWords =
            this.toneProfiles.get('encouraging')!.negativeWords;
      ...
      },
      {
        ...
        condition: metrics =>
          metrics.encouragingPhraseCount === 0 && metrics.positiveWordCount < 2,
        severity: 'warning',
      ...
      },
      {
        ...
        description:
          'Content should not be overly urgent unless truly critical',
  ...
  private initializeReplacementDictionary(): void {
    this.replacementDictionary.set('terrible', [
      'challenging',
      'needs attention',
      'could be improved',
    ]);
    this.replacementDictionary.set('awful', [
      'needs work',
      'could be better',
      'requires attention',
    ]);
    this.replacementDictionary.set('bad', [
      'needs improvement',
      'could be enhanced',
      'needs attention',
    ]);
    this.replacementDictionary.set('failed', [
      'needs retry',
      'requires attention',
      'could use work',
    ]);
    this.replacementDictionary.set('must', [
      'could',
      'might want to',
      'consider',
    ]);
    this.replacementDictionary.set('have to', ['could', 'might', 'consider']);
    this.replacementDictionary.set('required', [
      'recommended',
      'suggested',
      'beneficial',
    ]);
    this.replacementDictionary.set('you should have', [
      'next time consider',
      'it would help to',
      'you might',
    ]);
    this.replacementDictionary.set("you didn't", [
      'looks like',
      'seems like',
      'appears that',
    ]);
    this.replacementDictionary.set('you forgot', [
  ...
  private calculateMessageMetrics(
    content: NotificationContent
  ): MessageMetrics {
    ...
    return {
      ...
      positiveWordCount: this.countMatches(
        fullText,
        encouragingProfile.positiveWords
      ),
      negativeWordCount: this.countMatches(
        fullText,
        encouragingProfile.negativeWords
      ),
      encouragingPhraseCount: this.countMatches(
        fullText,
        encouragingProfile.encouragingPhrases
      ),
      discouragingPhraseCount: this.countMatches(
        fullText,
        encouragingProfile.discouragingPhrases
      ),
      urgencyWordCount: this.countMatches(
        fullText,
        encouragingProfile.urgencyWords
      ),
      pressureWordCount: this.countMatches(
        fullText,
        encouragingProfile.pressureWords
      ),
  ...
  private determineSentiment(
    ...
    const sentimentScore =
      metrics.positiveWordCount +
      metrics.encouragingPhraseCount -
      metrics.negativeWordCount -
      metrics.discouragingPhraseCount;
  ...
  private identifyTone(
    ...
    const professionalScore = this.countMatches(
      text,
      this.toneProfiles.get('professional')!.positiveWords
    );
    const casualScore = this.countMatches(
      text,
      this.toneProfiles.get('casual')!.positiveWords
    );
  ...
  private calculateEncouragementLevel(
    metrics: MessageMetrics,
    content: NotificationContent
  ): number {
    const encouragementScore =
      metrics.encouragingPhraseCount * 0.3 +
  ...
  private generateImprovementSuggestions(
    ...
    if (metrics.encouragingPhraseCount === 0) {
      suggestions.push(
        'Add encouraging phrases like "you\'ve got this" or "great work"'
      );
    }

    if (metrics.positiveWordCount < 2) {
      suggestions.push(
        'Include more positive words to enhance the encouraging tone'
      );
    }
  ...
  private async addEncouragingElements(
    ...
    const encouragingPhrases =
      this.toneProfiles.get('encouraging')!.encouragingPhrases;
