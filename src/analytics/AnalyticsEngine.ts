...
export class AnalyticsEngine {
  ...
  constructor(config: AnalyticsConfiguration) {
    ...
    this.stalenessDetector = new StalenessDetector(config.staleness, this.cache);
    ...
    this.workloadCalculator = new UserWorkloadCalculator(config.workload, this.cache);
    ...
    try {
      ...
      const [staleness, deadline, context, workload] = await Promise.all([
        ...
        this.config.general.enabledComponents.workload && userPreferences
          ? this.workloadCalculator.analyzeWorkloadImpact(issue, userPreferences)
          : Promise.resolve(this.getDefaultWorkloadImpact(issue))
      ]);
      
      // Calculate overall score and recommendation
      const overallScore = this.calculateOverallScore(staleness, deadline, context, workload);
      const recommendedAction = this.determineRecommendedAction(
        issue, staleness, deadline, context, workload, overallScore
      );
   ...
   */
  async batchAnalyzeIssues(request: BatchAnalysisRequest): Promise<BatchAnalysisResult> {
    const startTime = performance.now();
    ...
    try {
      ...
      const userPreferencesMap = await this.getUserPreferencesMap(Array.from(issues.values()));
      ...
      for (let i = 0; i < issueArray.length; i += batchSize) {
        ...
        switch (request.analysisType) {
          case 'full':
            const fullResults = await this.runFullBatchAnalysis(batch, userPreferencesMap);
          ...
          case 'workload_only':
            const workloadResults = await this.runWorkloadOnlyAnalysis(batch, userPreferencesMap);
   ...
   */
  async findIssuesNeedingAttention(options: {
    projectIds?: string[];
    assignees?: string[];
    maxResults?: number;
    ...
    const { projectIds, assignees, maxResults = 500, analysisTypes = ['staleness', 'deadlines'] } = options;
    ...
    const analysisPromises = issues.map(issue => {
      const userPrefs = issue.fields.assignee ? userPreferencesMap.get(issue.fields.assignee.accountId) : undefined;
    ...
    const analyses = (await Promise.all(analysisPromises)).filter(Boolean) as IssueAnalysisResult[];
    ...
    analyses.forEach(analysis => {
      if (analysis.overallScore >= 0.8 || analysis.recommendedAction.urgency === 'critical') {
        highPriority.push(analysis);
      } else if (analysis.overallScore >= 0.6 || analysis.recommendedAction.urgency === 'high') {
  ...
  async getProjectAnalytics(projectId: string): Promise<{
    ...
    const issues = await this.jiraClient.getProjectIssues(projectId, { maxResults: 1000 });
    const userPreferencesMap = await this.getUserPreferencesMap(issues);
    ...
    const overview = {
      ...
      overdueIssues: analyses.filter(a => a.deadline.daysUntilDue !== undefined && a.deadline.daysUntilDue < 0).length,
      highPriorityIssues: analyses.filter(a => a.context.priorityScore > 0.7).length
    };
    ...
    const recommendations = this.generateProjectRecommendations(analyses, overview);
  ...
  private async runStalenessOnlyAnalysis(
    ...
    try {
      const stalenessResults = await this.stalenessDetector.batchAnalyzeStaleness(issues);
      ...
      errors.push({
        component: 'staleness',
        error: error instanceof Error ? error.message : 'Staleness analysis failed',
        severity: 'error'
      });
    }
  ...
  private async runDeadlinesOnlyAnalysis(
    ...
    try {
      const deadlineResults = await this.deadlineMonitor.batchAnalyzeDeadlines(issues);
      ...
      for (const issue of issues) {
        ...
        if (deadline) {
          const result: IssueAnalysisResult = {
            ...
            context: this.getDefaultContextAnalysis(),
            workload: this.getDefaultWorkloadImpact(issue),
            overallScore: deadline.urgency === 'critical' ? 0.9 : deadline.urgency === 'high' ? 0.7 : 0.4,
            recommendedAction: {
              actionType: deadline.urgency === 'critical' ? 'priority_alert' : 'deadline_notification',
      ...
      errors.push({
        component: 'deadline',
        error: error instanceof Error ? error.message : 'Deadline analysis failed',
        severity: 'error'
      });
    }
  ...
  private async runWorkloadOnlyAnalysis(
    ...
    try {
      const workloadResults = await this.workloadCalculator.batchAnalyzeWorkloadImpact(issues, userPreferencesMap);
      ...
      for (const issue of issues) {
        ...
        if (workload) {
          const result: IssueAnalysisResult = {
            ...
            recommendedAction: {
              actionType: workload.shouldNotify ? 'workload_suggestion' : 'no_action',
              urgency: 'low',
              message: workload.notificationReason,
              suggestedActions: workload.shouldNotify ? ['Consider gentle reminder'] : ['No action needed'],
              timing: { 
                immediate: false, 
  ...
  private calculateOverallScore(
    ...
    if (this.config.general.enabledComponents.staleness) {
      const stalenessScore = staleness.isStale ? (staleness.staleness === 'abandoned' ? 1.0 : 0.7) : 0.2;
    ...
    if (this.config.general.enabledComponents.deadlines) {
      const deadlineScore = deadline.urgency === 'critical' ? 1.0 : 
                           deadline.urgency === 'high' ? 0.8 :
                           deadline.urgency === 'medium' ? 0.5 : 0.2;
    ...
    if (this.config.general.enabledComponents.context) {
      const contextScore = (context.priorityScore + context.typeScore + context.projectImportance) / 3;
  ...
  private determineRecommendedAction(
    ...
    if (deadline.urgency === 'critical' || overallScore >= 0.9) {
      ...
      urgency = 'critical';
      suggestedActions = ['Immediate attention required', 'Review and prioritize', 'Consider escalation'];
    } else if (deadline.urgency === 'high' || overallScore >= 0.7) {
      actionType = 'deadline_notification';
      urgency = 'high';
      suggestedActions = ['Review progress', 'Update status', 'Check dependencies'];
      ...
      urgency = 'low';
      suggestedActions = ['Consider when convenient', 'Review in next planning session'];
    }
    
    // Generate message
    const message = this.generateActionMessage(issue, staleness, deadline, actionType, urgency);
    
    // Determine timing
    const timing = {
      immediate: urgency === 'critical' || urgency === 'high',
      scheduledTime: urgency === 'low' ? workload.optimalNotificationTime : undefined,
      delayReason: !workload.shouldNotify ? workload.notificationReason : undefined
    };
  ...
  private async getUserPreferencesMap(issues: JiraIssue[]): Promise<Map<string, UserPreferences>> {
  ...
  private sortAnalysesByScore(analyses: IssueAnalysisResult[]): IssueAnalysisResult[] {
  ...
  private generateInsights(analyses: IssueAnalysisResult[]): {
    ...
    const averageScore = analyses.reduce((sum, a) => sum + a.overallScore, 0) / totalAnalyzed;
    const criticalCount = analyses.filter(a => a.recommendedAction.urgency === 'critical').length;
    ...
    if (staleCount > totalAnalyzed * 0.3) {
      recommendations.push('High number of stale issues detected - consider team review');
    }
    
    const overdueCount = analyses.filter(a => a.deadline.daysUntilDue !== undefined && a.deadline.daysUntilDue < 0).length;
    if (overdueCount > 0) {
      recommendations.push(`${overdueCount} overdue issues need timeline review`);
    }
  ...
  private calculateProjectTrends(analyses: IssueAnalysisResult[]): {
    ...
    analyses.forEach(analysis => {
      ...
      stalenessDistribution[staleness] = (stalenessDistribution[staleness] || 0) + 1;
      ...
      workloadDistribution[capacity] = (workloadDistribution[capacity] || 0) + 1;
    });
  ...
  private generateProjectRecommendations(
    ...
    if (overview.staleIssues / overview.totalIssues > 0.4) {
      recommendations.push('Consider implementing regular issue review sessions');
    }
    ...
    if (overview.highPriorityIssues / overview.totalIssues > 0.3) {
      recommendations.push('High concentration of priority issues - consider sprint planning review');
    }
    
    return recommendations;
  }

  private async generateTeamInsights(projectId: string, analyses: IssueAnalysisResult[]): Promise<{
    overloadedUsers: string[];
    underutilizedUsers: string[];
    collaborationScore: number;
  }> {
  ...
  private getDefaultStalenessAnalysis(): any {
    return {
      ...
      activityPattern: { weekdayActivity: [], hourlyActivity: [], activityTrend: 'stable', seasonalFactors: {} },
      isStale: false,
      staleness: 'fresh',
      confidence: 0.5,
      factors: {}
  ...
  private cacheAnalysisResult(issueKey: string, result: IssueAnalysisResult): void {
  ...
  private updatePerformanceMetrics(processingTime: number, success: boolean): void {
    ...
    if (this.performanceMetrics.analysisTime.average === 0) {
      ...
      this.performanceMetrics.analysisTime.min = 
        Math.min(this.performanceMetrics.analysisTime.min, processingTime);
      this.performanceMetrics.analysisTime.max = 
        Math.max(this.performanceMetrics.analysisTime.max, processingTime);
    }
  ...
  getCacheStats(): {
    ...
    return {
      ...
      totalSize: this.cache.issues.size + this.cache.users.size + 
                this.cache.projects.size + this.cache.analysis.size
    };
  }
}
