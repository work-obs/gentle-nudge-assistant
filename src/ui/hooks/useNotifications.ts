...
export const useNotifications = (userId: string): UseNotificationsReturn => {
  ...
  const saveNotifications = useCallback(async (updatedNotifications: NotificationMessage[]) => {
    try {
  ...
  const actionNotification = useCallback(async (id: string, issueKey?: string) => {
    const updatedNotifications = notifications.filter(notification => notification.id !== id);
    await saveNotifications(updatedNotifications);
    ...
    try {
      ...
      if (targetIssueKey) {
        ...
        await storage.set(trackingKey, {
          ...
          lastNudgeDate: new Date(),
          nudgeCount: (existingTracking.nudgeCount || 0) + 1,
          effectivenessScore: Math.min((existingTracking.effectivenessScore || 0) + 0.3, 1.0)
        });
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('gentle-nudge-notification', handleNewNotification as EventListener);
    
    return () => {
      window.removeEventListener('gentle-nudge-notification', handleNewNotification as EventListener);
    };
  }, []);
