...
export class StalenessDetector {
   ...
   */
  async batchAnalyzeStaleness(issues: JiraIssue[]): Promise<Map<string, StalenessAnalysis>> {
  ...
  private determineStalenessLevel(
    ...
    const adjustedDays = daysSinceUpdate / (typeMultiplier * priorityMultiplier);
   ...
   */
  private async analyzeStalenessFactors(issue: JiraIssue): Promise<StalenessFactors> {
    ...
    const projectActivity = await this.analyzeProjectActivity(issue.fields.project.id);
   ...
   */
  private async analyzeAssigneeActivity(userId: string): Promise<AssigneeActivity> {
    // Check cache first
    ...
    if (cached && cached.expiresAt > new Date()) {
      return cached.workload.stressIndicators.overallStressLevel === 'low' 
        ...
        : {
            ...
            workloadLevel: cached.workload.workloadCapacity as AssigneeActivity['workloadLevel']
          };
    }
   ...
   */
  private async analyzeProjectActivity(projectId: string): Promise<ProjectActivity> {
    const cached = this.cache.projects.get(projectId);
    if (cached && cached.expiresAt > new Date()) {
      const teamMetrics = cached.teamMetrics;
      return {
        overallActivityScore: teamMetrics.teamCapacity === 'healthy' ? 0.8 : 0.4,
        teamSize: 8, // Would be derived from actual data
        recentIssueUpdates: teamMetrics.totalActiveIssues,
        projectHealth: teamMetrics.teamCapacity === 'healthy' ? 'healthy' : 'moderate'
      };
    }
   ...
   */
  private async analyzeActivityPattern(issue: JiraIssue): Promise<ActivityPattern> {
  ...
  private async hasRecentWorklogs(issue: JiraIssue): Promise<boolean> {
    if (issue.fields.worklog && issue.fields.worklog.worklogs.length > 0) {
      const latestWorklog = issue.fields.worklog.worklogs
  ...
  private async hasRecentStatusChanges(issue: JiraIssue): Promise<boolean> {
    ...
    return this.calculateDaysDifference(new Date(issue.fields.updated), new Date()) <= 5;
  }
  ...
  private async getDaysSinceLastComment(issue: JiraIssue): Promise<number> {
    ...
    return this.calculateDaysDifference(new Date(issue.fields.updated), new Date());
  }
  ...
  private async getDaysSinceLastWorklog(issue: JiraIssue): Promise<number> {
    if (issue.fields.worklog && issue.fields.worklog.worklogs.length > 0) {
      ...
      return this.calculateDaysDifference(new Date(latestWorklog.created), new Date());
    }
    
    // If no worklogs, use creation date as baseline
    return this.calculateDaysDifference(new Date(issue.fields.created), new Date());
  }
  ...
  private isHolidayPeriod(date: Date): boolean {
    ...
    const isChristmasNewYear = (month === 11 && day > 20) || (month === 0 && day < 10);
